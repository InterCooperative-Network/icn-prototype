...
def register():
    ...
    return jsonify({"message": f"User {username} registered successfully", "did": did_id}), 201
...
def login():
    ...
    access_token = create_access_token(identity={"username": username, "role": user['role'], "did": user['did']})
...
def complete_order():
    ...
    if completed_order:
        return jsonify({"message": "Order completed", "order": completed_order.__dict__}), 200
