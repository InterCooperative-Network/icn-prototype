...
class Node:
    ...
    def enter_cooldown(self, cooldown_period: int) -> None:
        ...
        logger.info(f"Node {self.node_id} entered cooldown for {cooldown_period} periods")
    ...
    def update_metrics(self, metrics: Dict[str, float]) -> None:
        ...
        if self.total_validations > 0:
            success_rate = len([v for v in self.validation_history 
                              if v.get('evidence', {}).get('success', False)
                             ]) / self.total_validations * 100
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'active_shards': {
                k: v.isoformat() for k, v in self.active_shards.items()
            },
